App.controller.define('CMain', {
	...
	, recordAffectation: function (me) {
		...
		if (cboFournisseur.getRawValue() == cboFournisseur.getValue()) {
			...
			App.DB.post('sysiphe://fournisseurs', {
				FOURNISSEUR: cboFournisseur.getValue()
			}, function (e, r) {
				console.log(e);
				if (e.insertId) cboFournisseur.setValue(e.insertId);
				else cboFournisseur.setValue('');
			});
		};
		//return;
			...
			App.Docs.upload(App.get('uploadfilemanager#up').getFiles(), 0, function () {
				...
				App.DB.get('sysiphe://affectations?IDAFFECTATION=' + App.get(me.up('window'), 'textfield#IDAFFECTATION').getValue(), function (r) {
					if (r.data.length > 0) {
						...
						if ((!Post.IDSYSIPHE) && (!Post.IDUTILISATEUR)) _exit();
						else {
	...
	, cboFamille_select: function (me) {
		...
		App.get(me.up('window'), 'combo#cboMarque').setValue('');
	...
	, VAffectation_show: function (me) {
		if (me._data) {
			...
			if (me._data.SENSIBLE) App.get(me, 'radiogroup#r1').setValue({
				rc: 1
			});
			...
			if (me._data.IDUTILISATEUR == 0) {
				...
				var store = App.store.create('bpclight://agents{Kage,Kuni,Ksub,Nom,Prenom,Nom+" "+Prenom=NomPrenom+}', {
					autoLoad: true
				});
	...
	, filter_onclick: function () {
		...
		else App.get('FilterBox#FilterPanel').show();
	}
	, onLoad: function () {
		Auth.login(function() {
